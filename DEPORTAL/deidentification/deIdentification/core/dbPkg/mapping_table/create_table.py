...
class MappingTable:
    def __init__(self, source_connection_str: str, config: MappingConfig, client_id: int, dump_id: int, client_type: str):
        self.config = config
    ...
    def filter_existing_patients(self, dfs, existing_df):
        ...
        for col in self.identifier_columns:
            if col in existing_df.columns:
                # Get non-null values and convert to set for fast lookup
                existing_identifier_sets[col] = set(existing_df[col].dropna().astype(str))
            else:
        ...
        for df in dfs:
            ...
            for col in self.identifier_columns:
                if col in df.columns:
                    # Check if any value in this column matches existing patients
                    col_mask = df[col].notna() & df[col].astype(str).isin(existing_identifier_sets[col])
                    mask = mask | col_mask
    ...
    def generate_encounter_mapping_table(self):
        ...
        if not only_new.empty:
            ...
            nd_logger.info(
                f"Client: {self.client_id}, Dump: {self.dump_id} - Appended {len(only_new)} new encounters to {ENCOUNTER_MAPPING_TABLE}."
            )
        else:
            nd_logger.info(f"Client: {self.client_id}, Dump: {self.dump_id} - No new encounters to update.")
