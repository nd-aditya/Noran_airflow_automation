...
class DeIdentifier:
    ...
    def apply_rules(self) -> pd.DataFrame:
        ...
        for column_config in notes_configs:
            nd_logger.info("####################################################")
            nd_logger.info(f"Applying NotesRule for column: {column_config['column_name']}, dump-id: {self.dump_obj.id}, table-id: {self.table_obj.id}")
            
            if self._notes_rule is None:
                from .unstruct.notes import NotesRule  # local import to avoid circular issues
                self._notes_rule = NotesRule(self.dump_obj, self.key_phi_columns, self.possible_patient_identifier_columns, self.table_obj)
        ...
        for column_config in other_configs:
            nd_logger.info("####################################################")
            nd_logger.info(f"Applying for column: {column_config['column_name']}, dump-id: {self.dump_obj.id}, table-id: {self.table_obj.id}")

        pii_config = self.dump_obj.get_pii_config()
        # Process all other rules
        for column_config in other_configs:
            nd_logger.info("####################################################")
