...
import traceback
from nd_api.models import ClientDataDump, Table as TableModel, Clients
...
class JoinMapping:
    def __init__(self, df, key_phi_columns, table_obj: TableModel):
        self.df = df
        self.key_phi_columns = key_phi_columns
        dump_obj = table_obj.dump
        client: Clients = dump_obj.client
        self.mapping_db_config = dump_obj.get_mapping_db_config()
        nd_logger.info(
            f"[{self.__class__.__name__}] Initialized with table: {table_obj.table_name}"
        )
...
def start_de_identification_for_table_with_df(
    ...
    try:
        ...
        dump_obj: ClientDataDump = table_obj.dump

        nd_logger.info(f"getting connection for source db: {table_obj.table_name}")
        source_db_connection: NDDBHandler = dump_obj.get_source_db_connection()

        read_start = time.time()
        nd_logger.info(f"Reading the rows from table: {table_obj.table_name}")
        df = source_db_connection.get_table_as_dataframe(table_obj.table_name, batch_size, offset)
        ...
        df = df.drop_duplicates(subset=['nd_auto_increment_id'])
        ...
        row_handler = InvalidRowHandler(dump_name=table_obj.dump.dump_name, table_name=table_obj.table_name, key_phi_columns=key_phi_columns)
        ...
        deidentifier = DeIdentifier(df=df, config=table_config["columns_details"],table_obj=table_obj,
            dump_obj=dump_obj, key_phi_columns=key_phi_columns,possible_patient_identifier_columns=possible_patient_identifier_columns)

        df = deidentifier.apply_rules()


        destination_db: NDDBHandler = dump_obj.get_destination_db_connection()
        column_schema_mapping = _get_columns_schema_mapping(table_config)
        source_db_connection.create_table_in_dest_if_not_exists(
            table_obj.table_name, destination_db, column_type_mapping=column_schema_mapping
        )
        destination_db.insert_dataframe_in_batches(df, table_name=table_obj.table_name)
        ...
        prepare_message = {
            ...
            "table_identifier": f"{table_id} - {table_obj.table_name}",
            "dump_identifier": f"{dump_obj.id} - {dump_obj.dump_name}",
            "client_identifier": f"{dump_obj.client.id} - {dump_obj.client.client_name}",
