...
class InvalidRowHandler:
    def __init__(self, dump_name: str, table_name: str, key_phi_columns: tuple):
        self.dump_name = dump_name
        self.table_name = table_name
        self.key_phi_columns = key_phi_columns

        nd_logger.info(
            f"[InvalidRowHandler] Initialized for db: '{dump_name}', table: '{table_name}'"
        )
            ...
            try:
                ...
                rows_to_save.append(
                    IgnoreRowsDeIdentificaiton(
                        dump_name=self.dump_name, table_name=self.table_name, row=row_json
                    )
                )
