...
from nd_api.models import ClientDataDump, Table as TableModel
from core.process_df.unstruct.xml import deidentify_xml_tags
from core.process_df.unstruct.xml_utils import xml_tag_replacements
...
class NotesRule(RuleBase):
    def __init__(self, dump_obj: ClientDataDump, key_phi_columns: tuple, possible_patient_identifier_columns: list, table_obj: TableModel):
        self.table_obj: TableModel = table_obj
        self.pii_config = dump_obj.get_pii_config()
        self.secondary_pii_configs = dump_obj.get_secondary_pii_configs()
    ...
    def de_identify_key_phi_columns(
        ...
        encounter_id_cols, patient_id_cols, appointment_id_cols = self.key_phi_columns
        ...
        patient_idfr_cols = []
        for key, value in patient_id_cols.items():
            patient_idfr_cols.extend(value)

        nd_logger.info(
            f"[{self.__class__.__name__}] De-identifying using encounter_id_col={encounter_id_col}, "
            f"patient_id_cols={patient_idfr_cols},"
        )
        ...
        def replace_row(row):
            ...
            for col in patient_idfr_cols:
                _new_col = f"_resolved_{col}"
                if _new_col and pd.notnull(row.get(_new_col)):
                    original = str(row[_new_col])
    ...
    def _apply_mask_batched_fuzzy_matching(
        ...
        mask_config = self.pii_config.get("config", {}).get("mask", {})
        ...
        def fuzzy_replace_row(text, pii_values, mask_config):
            ...
            for col, pii_value in pii_values.items():
                masking_value = mask_config[col]["masking_value"]
                ...
                for match_text in fuzzy_matches:
                    ...
                    result_text = pattern.sub(masking_value, result_text)
    ...
    def _apply_dob(self, df_batch: pd.DataFrame, masked_col: pd.Series) -> pd.Series:
        ...
        dob_config = self.pii_config.get("config", {}).get("dob", {})
        if not dob_config:
            nd_logger.info(
        ...
        def replace_dates_in_text(text: str, replacements: dict) -> str:
            ...
            def normalize_two_digit_year(d: date) -> date:
                today = date.today()
                if d > today:
            ...
            def replacer(match):
                ...
                try:
                    ...
                    parsed_date = normalize_two_digit_year(parsed_date)
    ...
    def _build_rowwise_patterns(self, df_batch: pd.DataFrame) -> dict:
        combine_config = self.pii_config.get("config", {}).get("combine", {})
        ...
        for rule_name, rule in combine_config.items():
            ...
            nd_logger.debug(
                f"[{self.__class__.__name__}] [Combine:{rule_name}] Masking value: {masking_value}"
            )
    ...
    def _apply_regex(self, masked_col: pd.Series) -> pd.Series:
        ...
        regex_config = self.pii_config.get("config", {}).get("regex", {})
        if not regex_config:
            nd_logger.info(
    ...
    #     )
    #     replace_rules = self.pii_config.get("config", {}).get("replace_value", [])
