...
class DataGenerator:
    ...
    def get_source_sample_in_batches(
        self, engine: Engine, table_name: str, read_limit: int, read_offset: int
    ) -> pd.DataFrame:
        """
        ...
        if dialect in ("mssql", "microsoft"):
            ...
            """)
            params = {"limit": read_limit, "offset": read_offset}
    ...
    def generate_sample(
        ...
        source_df = self.get_source_sample_in_batches(
            self.source_engine, table_name, read_limit, read_offset
        )
        dest_df = self.get_dest_sample_in_batches(
            self.dest_engine, table_name, source_df
        )
        return read_limit, source_df, dest_df
