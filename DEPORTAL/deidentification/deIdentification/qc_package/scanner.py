...
from nd_api.models import IgnoreRowsDeIdentificaiton, Table as TableModel, Status
...
class LoadMappingData:
    ...
    def load(cls, source_df: pd.DataFrame, table_obj: TableModel):
        nd_logger.info(f"[LoadMappingData] Loading mapping data for table: {table_obj.table_name}")
        available_identifiers = {}
        end_ndids = []
        for column_conf in table_obj.table_details_for_ui["columns_details"]:
        ...
        m_conn_str = table_obj.dump.get_mapping_db_connection_str()
...
class LoadPIIData:
    ...
    def load(cls, patient_mapping_df:pd.DataFrame, encounter_mapping_df: pd.DataFrame, table_obj: TableModel):
        nd_logger.info(f"[LoadPIIData] Loading PII data for table: {table_obj.table_name}")
        def get_all_ndids():
            series_list = []
        ...
        # --- Load PII Data ---
        pii_config = table_obj.dump.get_pii_config()
        ...
        secondary_dfs = []
        secondary_config = table_obj.dump.get_secondary_pii_configs()
...
class DbScanner:
    ...
    def get_structured_detectors(
        ...
        nd_logger.info(f"[DbScanner] Getting structured detectors for table: {table_obj.table_name}")
        detectors = []
        ...
        for col_conf in table_obj.table_details_for_ui["columns_details"]:
            if col_conf["is_phi"] and col_conf["de_identification_rule"] not in [
                ...
                patient_identifiers = get_patient_identifiers(table_obj.table_details_for_ui)
    ...
    def scan_table(
        ...
        important_cols = DbScanner.get_important_columns(table_obj.table_details_for_ui)
        src_conn_str = table_obj.dump.get_source_db_connection_str()
        dest_conn_str = table_obj.dump.get_destination_db_connection_str()
        ...
        try:
            with transaction.atomic(savepoint=CREATE_SAVEPOINT_IN_TRANSACTION):
                ...
                nd_logger.info(f"[DbScanner] Created Chain (id={chain.id}, created={created}) for QC on table {table_obj.table_name}")
                if not created:
                    chain.revive_and_save()
                    nd_logger.info(f"[DbScanner] Revived pre-existing Chain for table {table_obj.table_name}")
                qc_tasks_list = []
                for read_offset in range(0, table_obj.rows_count, read_limit):
                    nd_logger.debug(f"[DbScanner] Creating QC task at offset {read_offset} for table {table_obj.table_name}")
...
def qc_task(
    ...
    nd_logger.info(f"[qc_task] QC task started for table_id={table_id}, read_offset={read_offset}, limit={read_limit}")
    table_obj = TableModel.objects.get(id=table_id)
    try:
        ...
        read_limit, source_df, dest_df = data_generator.generate_sample(
            table_obj.table_name, read_limit, read_offset, important_cols
        )
        ...
        pii_config = table_obj.dump.pii_config
        ...
        table_obj.qc.save()
        prepare_message = {
            "alert_type": f"QC Failed for the table: {table_obj.table_name}, due to code failure",
            "dump_identifier": f"{table_obj.dump.id} - {table_obj.dump.dump_name}",
            "client_identifier": f"{table_obj.dump.client.id} - {table_obj.dump.client.client_name}",
            "traceback": traceback.format_exc()
        }
...
def qc_cleanup_task(
    ...
    try:
        ...
        ignore_row_count = IgnoreRowsDeIdentificaiton.objects.filter(
            dump_name=table_obj.dump.dump_name, table_name=table_obj.table_name
        ).count()
        dest_count = dest_handler.get_rows_count(table_obj.table_name)
        data_count_result = {
            "source_rows_count": int(table_obj.rows_count) if table_obj.rows_count is not None else 0,
        ...
        if table_obj.qc.qc_status == Status.FAILED:
            prepare_message = {
                "alert_type": f"QC Failed for the table: {table_obj.table_name}, Due to Improper deidentification",
                "dump_identifier": f"{table_obj.dump.id} - {table_obj.dump.dump_name}",
                "client_identifier": f"{table_obj.dump.client.id} - {table_obj.dump.client.client_name}",
                "traceback": str(traceback.format_exc())
            }
            alert_sender.send_message(prepare_message)
            nd_logger.warning(f"[qc_cleanup_task] Sent alert: QC failed for table {table_obj.table_name}")
...
def send_qc_websockte_event(table_obj: TableModel):
    ...
    """
    nd_logger.debug(f"[send_qc_websockte_event] Attempting to send websocket event for table_id={table_obj.id}, table_name={table_obj.table_name}")
    ...
    broadcast_table_status_update(
        ...
        process_type='qc',
        status=ui_status,
        message=f"QC {ui_status} for table {table_obj.table_name}",
